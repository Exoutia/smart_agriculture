...
char ssid[] = "Hail Hydra";
char pass[] = "Bournvita@#$123";
...
void loop() {
  ...
  // Blynk switch control for pump
  BLYNK_WRITE(V1) {
    pump_flag = param.asInt();
  }

  
  while (m1_moi < threshold && pump_flag == 1) {
    myservo.write(0);
  ...
  while (m2_moi < threshold && pump_flag == 1) {
    myservo.write(90);
    digitalWrite(pump, HIGH);
  ...
  while (m3_moi < threshold && pump_flag == 1) {
    myservo.write(180);
    digitalWrite(pump, HIGH);
...
float moisture_val(float val, int num) {
  float _moisture =(100 - ((val/4095.00)*10
